%%
% Spring 2024 Senior Project LiDAR Cloud Fourier Transform MATLAB file
% 
% Authors: Andy Clarkson, Samuel Adams, Justin Ackerman, Brighton
% Rosskelly, Tyler Brady

% Purpose: Take 2 columns of a distance measured/exported into a .csv/.xlsx
% file (right now it only is set to take a .csv) and measure the most
% frequently measured distances.
%%
% Read the signal data from an Excel file
filename = ['Lidar_July16_test_data.csv']; % Replace with your Excel file name
...
%     plot(f(locs), pks, 'r*'); % Mark peaks
    title('Binned Magnitude Spectrum (First & Last Bin Removed)');
    xlabel('Frequency (Hz)');
    ylabel('Magnitude');
